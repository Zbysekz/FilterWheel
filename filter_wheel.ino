//compile for Generic ESP8266
//AccelStepper v1.64.0 by Patrick Wasp: https://www.airspayce.com/mikem/arduino/AccelStepper/ 
#include <AccelStepper.h> //stepper library
#include <EEPROM.h>       //eprom library

// Constants
#define SENSOR 14    // Hall senser pin
#define hallPower 12 // hall 5V pin
#define hallCom 13   // hall GND pin
#define upButton 0    // UpButton Pin
#define downButton 2  // Down BUtton Pin
#define maxSpeed 8000  // max motor speed
#define Speed 300     // current speed
#define setAccel 7000 // stepper accelaration
#define buzzerPin 2
#define ledPin 2

// SETTINGS - MAIN PARAMETERS
#define stepsPerRevolution 84000 //set number motor steps for wheel to rotate once
#define numberOfFilters 7
#define offSetResolution 50 //number of motor steps for one offset change
#define homingOffsetSteps 6325 // number of steps from homing point to centre of first filter
#define buzzerConnected 0 //1 to enable on pin 2, 0 to disable
#define ledConnected 0 //1 to enable on pin 3, 0 to disable 

// SETTINGS - MOTOR

#define IN1 15  // D8 GPIO15 - unipolar motor pins
#define IN2 16  // D0 GPIO16 - motor driver step pin  
#define IN3 15  // D8 GPIO15
#define IN4 5   // D1 GPIO5  - motor driver dir pin
#define motorType 2 //1 for unipolar, 2 for motor drive board (e.g. A4988)
#define motor_EN 4  // D2 GPIO4
...
const String I_Strings[] = {"Zapadlik's_Filter_1.0", "FW1.0.0", "P", "S/N:001", "Max Speed " + String(maxSpeed), "Jitter 1", "PX Offset ", "Threshold 1", "FilterSlots " + String(numberOfFilters), "Pulse Width 4950uS"};
...
{ // got to new position.
  ...
  /*bool dir=false;
  int steps = 0;
  if(abs(newPos-currPos)>=5){
    dir=newPos>currPos;
    steps = numberOfFilters - abs(newPos-currPos);
  }else{
    dir=newPos<currPos;
    steps = abs(newPos-currPos);
  }*/
  //int steps = abs(newPos-currPos);
  //Serial.println("dir:%s",(dir?"T":"F"));
  //Serial.println("steps:%d",steps);
  
  //if(!dir && currPos+steps>7){
    //if we are going up and we will cross start
  //  Serial.println("intermpos");
  //  stepper.runToNewPosition(filterPos[1] + posOffset[1 - 1]);
  //}
  /*stepper.moveTo(filterPos[newPos] + posOffset[newPos - 1]);
  long dtg = stepper.currentPosition();
  while(stepper.distanceToGo() != 0) {
    stepper.run(); // Move one step toward target
    long pos = stepper.currentPosition(); // Get current (intermediate) position
    if(abs(dtg - pos) > 50000){
...
{                             // runs once.
  ...
  pinMode(SENSOR, INPUT_PULLUP);     // hall sensor pin as input
  pinMode(IN1, OUTPUT);       // motor pins as outputs.
  pinMode(IN2, OUTPUT);
  pinMode(IN3, OUTPUT);
  pinMode(IN4, OUTPUT);
  //motor_EN is set by enableOutputs()
  
  pinMode(upButton, INPUT_PULLUP); // button pins as input with pull up resistor (active low).
  ...
  stepper.setPinsInverted(false,false,false,false, true); //enable pin is inverted
  stepper.enableOutputs();
